					 ...
					 unsigned long addr, const char *name)
{
	...
	if (strcmp(vma->vm_file->f_path.dentry->d_name.name, LIBC_SO) == 0)
		ret = true;
	else
...
bool handle_chp_prctl_user_addrs(const char __user *uname, unsigned long start,
				 unsigned long len)
{
	char *name;
	bool ret = false;
	...
	if (unlikely(!config_anon_enable || !uaddr ||
